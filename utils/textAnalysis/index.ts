...
export const analyzeText = (text: string): AnalysisResult => {
  ...
  // 2. Check Content Structure (Very important for AI detection)
...
const calculatePatternScores = (text: string) => {
  let score = 0;
...
const analyzeLanguagePatterns = (text: string) => {
  let score = 0;
  const findings: string[] = [];
...
const normalizeScore = (score: number): number => {
  // Amplify the score but keep it between 0 and 1
  return Math.min(score * 1.5, 1);
};
// Add this before calculateConfidence
const hasStatisticalPatterns = (text: string): boolean => {
  ...
  // Check if any statistical pattern is present
